...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    elseif ($action === 'get_stats') {
        ...
        if ($row = mysqli_fetch_assoc($res)) $stats['active_users'] = $row['c'];
        
        $res = mysqli_query($conn, "SELECT COUNT(*) as c FROM logs");
        if ($row = mysqli_fetch_assoc($res)) $stats['total_logs'] = $row['c'];

        $res = mysqli_query($conn, "SELECT COUNT(*) as c FROM logs WHERE action = 'Access Granted' AND DATE(timestamp) = CURDATE()");
        if ($row = mysqli_fetch_assoc($res)) $stats['entries_today'] = $row['c'];
    ...
    elseif ($action === 'get_orgs') {
        ...
        $items = []; while($row = mysqli_fetch_assoc($res)) $items[] = $row;
